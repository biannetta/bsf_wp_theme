...
function capacious_body_classes( $classes ) {
   ...
   if  ( is_archive() || is_author() || is_category() || is_home() || is_single() || is_tag())  {
           $imagedesignlayout = esc_attr( capacious_get_option( 'capacious_image_layout_option'));
           $classes[] = $imagedesignlayout;
         }
 // Adds a class on body.
    if  ( is_archive() || is_author() || is_category() || is_home() || is_single() || is_tag() || is_front_page())  {
           $button_design = esc_attr(capacious_get_option( 'capacious_button_design_layout_option'));
           $classes[] = $button_design;
         }
